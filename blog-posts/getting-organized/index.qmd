...
I use the core `Daily notes` and `Templates` extensions, and also `tasks` and `templater` community extensions.  I set up a daily note template that is very regimented. _Bonus - ask Claude to help setup the template_
...
The _Previous Tasks_ uses the `tasks` extension to pull in things I didn't finish to the top.  And I have separate files for each meeting, but this gives me a place to quickly take notes, then add them to the meeting note later.  
